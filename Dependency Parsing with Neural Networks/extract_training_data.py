...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        rep_vector = np.zeros((6,), dtype=int)

        # stack
        if len(state.stack) == 0:
            rep_vector[0:3] = self.word_vocab["<NULL>"]

        elif len(state.stack) == 1:
            rep_vector[0] = self.word_vocab["<ROOT>"]
            rep_vector[1:3] = self.word_vocab["<NULL>"]

        elif len(state.stack) == 2:
            rep_vector[1] = self.word_vocab["<ROOT>"]
            rep_vector[2] = self.word_vocab["<NULL>"]

            if pos[state.stack[-1]] == "CD":
                rep_vector[0] = self.word_vocab["<CD>"]
            elif pos[state.stack[-1]] == "NNP":
                rep_vector[0] = self.word_vocab["<NNP>"]
            elif words[state.stack[-1]].lower() not in self.word_vocab.keys():
                rep_vector[0] = self.word_vocab["<UNK>"]
            else:
                rep_vector[0] = self.word_vocab[words[state.stack[-1]].lower()]

        elif len(state.stack) == 3:
            rep_vector[2] = self.word_vocab["<ROOT>"]

            for i in [-1,-2]:
                if pos[state.stack[i]] == "CD":
                    rep_vector[-1 - i] = self.word_vocab["<CD>"]
                elif pos[state.stack[i]] == "NNP":
                    rep_vector[-1 - i] = self.word_vocab["<NNP>"]
                elif words[state.stack[i]].lower() not in self.word_vocab.keys():
                    rep_vector[-1 - i] = self.word_vocab["<UNK>"]
                else:
                    rep_vector[-1 - i] = self.word_vocab[words[state.stack[i]].lower()]
        else:
            for i in [-1,-2, -3]:
                if pos[state.stack[i]] == "CD":
                    rep_vector[-1 - i] = self.word_vocab["<CD>"]
        ...
        if len(state.buffer) == 0:
            rep_vector[3:6] = self.word_vocab["<NULL>"]

        elif len(state.buffer) == 1:
            rep_vector[4:6] = self.word_vocab["<NULL>"]

            if state.buffer[-1] == 0:
                rep_vector[3] = self.word_vocab["<ROOT>"]
            elif pos[state.buffer[-1]] == "CD":
                rep_vector[3] = self.word_vocab["<CD>"]
            elif pos[state.buffer[-1]] == "NNP":
                rep_vector[3] = self.word_vocab["<NNP>"]
            elif words[state.buffer[-1]].lower() not in self.word_vocab.keys():
                rep_vector[3] = self.word_vocab["<UNK>"]
            else:
                rep_vector[3] = self.word_vocab[words[state.buffer[-1]].lower()]

        elif len(state.buffer) == 2:
            rep_vector[5] = self.word_vocab["<NULL>"]

            for i in [-1,-2]:
                if state.buffer[i] == 0:
                    rep_vector[2 - i] = self.word_vocab["<ROOT>"]
                elif pos[state.buffer[i]] == "CD":
                    rep_vector[2 - i] = self.word_vocab["<CD>"]
                elif pos[state.buffer[i]] == "NNP":
                    rep_vector[2 - i] = self.word_vocab["<NNP>"]
                elif words[state.buffer[i]].lower() not in self.word_vocab.keys():
                    rep_vector[2 - i] = self.word_vocab["<UNK>"]
                else:
                    rep_vector[2 - i] = self.word_vocab[words[state.buffer[i]].lower()]
    ...
    def get_output_representation(self, output_pair):  
        ...
        if transition == "left_arc":
            ind = dep_relations.index((label))
        if transition == "right_arc":
            ind = dep_relations.index((label)) + 45
        if transition == "shift":
            ind = 90

        return keras.utils.to_categorical(ind, num_classes=91)

        # Another option to use np.zeros to initialize the one-hot vector
        # output_vector = np.zeros((91,), dtype=int)
        # if transition == "left_arc":
        #     ind = dep_relations.index((label))
        #     output_vector[ind] = 1
        # if transition == "right_arc":
